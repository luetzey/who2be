...
#   3) JWT-authentifizierter API-Aufruf (gen_test_jwt.py) → 200 fuer /v1/personas
...
HTTP_CODE="$(curl -sS -o /tmp/smoke-personas.json -w '%{http_code}' \
  -H "Authorization: Bearer ${TOKEN}" \
  "${API_URL}/v1/personas")"
[[ "${HTTP_CODE}" == "200" ]] || fail "/v1/personas lieferte ${HTTP_CODE}: $(cat /tmp/smoke-personas.json)"
