...
| Binaer-Originale (Blobs) | Dateiinhalte beliebiger Art (PDF/Text/HTML), `sha256`, `size_bytes`, `media_type`, `source_url` | Katalog `wa_blob` (Postgres) + **Objekte im BlobStore** (SeaweedFS/S3, selbst gehostet — Nachfolger von MinIO, s. ADR-0048-Addendum 2026-09-19; Key `blobs/{workspace_id}/{sha256}`) | `migrations/0075`; ADR-0048 |
...
| Server-Logs (Caddy-Access-Log) | **14 Tage**, durchgesetzt von der Log-Rotation selbst (`roll_keep_for 336h` in `deploy/hetzner/Caddyfile`), zusaetzlich groessenbegrenzt (`roll_size`/`roll_keep`). Container-Logs (stdout/stderr) je Dienst auf 3 x 10 MB gedeckelt (`logging:` in beiden Hetzner-Compose-Dateien) |
