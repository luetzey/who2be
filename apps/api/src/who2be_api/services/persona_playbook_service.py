...
ersetzt den Stand vollstaendig (PUT-Semantik); die Owner-Pruefung erfolgt
atomar im Repository.
"""
...
class PersonaPlaybookService:
    ...
    async def list_links(
        ...
        if not await self._repo.persona_belongs_to(owner_id, persona_id):
            raise _persona_not_found()
        return await self._repo.list_linked(persona_id)

    async def set_links(
        self, owner_id: UUID, persona_id: UUID, data: PersonaPlaybookLinkSet
    ) -> list[PlaybookRead]:
        ...
        result = await self._repo.set_links(owner_id, persona_id, ids)
