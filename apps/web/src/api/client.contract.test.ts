...
test('jeder vom Web-Client genutzte Pfad existiert im OpenAPI-Golden', async () => {
  ...
  const invocations: Array<Promise<unknown>> = [
    ...
    api.listExternalTools(),
    api.getExternalTool(ID),
    api.createExternalTool({} as never),
    api.updateExternalTool(ID, {} as never),
    api.patchExternalToolDraft(ID, {} as never),
    api.listExternalToolVersions(ID),
    api.transitionExternalToolVersion(ID, 7, 'active'),
    api.restoreExternalToolVersion(ID, 7),
    api.provenanceExternalToolVersion(ID, 7),
    api.deleteExternalTool(ID),
    api.exportExternalTool(ID, 'json'),
    ...
    // ADR-0047 — Arbeitsbereich + Knowledge Base.
    api.listWorkAreas(),
    api.createWorkArea({} as never),
    api.listWorkAreaGrants(ID),
    api.setWorkAreaGrant(ID, ID, { level: 'read' }),
    api.deleteWorkAreaGrant(ID, ID),
    api.listWaArtifacts(ID),
    api.readWaArtifact(ID),
    api.deleteWaArtifact(ID),
    api.exportWaArtifact(ID, 'markdown'),
    api.listWaTables(ID),
    api.describeWaTable(ID),
    api.queryWaTable(ID, { sql: 'select 1' }),
    api.exportWaTable(ID, 'csv'),
    api.searchWorkArea({ q: 'x' }),
    api.searchKb({ q: 'x' }),
    api.getKbNode(ID),
