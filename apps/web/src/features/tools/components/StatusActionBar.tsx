...
// Spiegelt `features/resources/components/StatusActionBar.tsx` 1:1.
export function StatusActionBar({
  ...
  const transition = async (to: VersionStatus, success: string) => {
    ...
    try {
      await api.transitionExternalToolVersion(toolId, version, to)
      ...
      if (missing !== null) {
        setPromoteError(
          t('statusBar.promoteError', { fields: formatMissingFields(missing) }),
        )
      } else {
        const message = cause instanceof Error ? cause.message : t('statusBar.actionFailed')
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        {showPromote ? (
          <Button
            ...
            title={canPromote ? undefined : t('statusBar.activateAdminOnly')}
          >
      ...
      {promoteError !== null ? (
        <ErrorAlert message={promoteError} title={t('statusBar.promoteErrorTitle')} />
      ) : null}
    </div>
  )
}
